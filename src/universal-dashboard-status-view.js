...
(function(root){
 ...
 function install(client){
   ...
   async function isFormAdmin(code){
     // Do not infer Delete authority from payment permissions, user labels, or client-side roles.
     // A failed or unavailable permission check must never expose the control.
     try{
       const {data,error}=await client.rpc('meg_forms_user_can_admin_delete',{p_form_code:code});
       return !error&&data===true;
     }catch(error){root.console?.warn?.('Admin Delete permission check unavailable',error);return false;}
   }
   ...
   async function deleteClaim(code,id,button){
     ...
     try{
       if(!await isFormAdmin(code))throw new Error('Form Admin authority required. No claim deleted.');
       ...
       if(!await isFormAdmin(code))throw new Error('Form Admin authority changed. No claim deleted.');
       button.textContent='Deleting…';
       // The database RPC independently enforces form-admin permission and deletion safeguards.
     ...
     }catch(error){root.alert(error?.message||'Admin Delete failed.');}
     finally{pending.delete(key);button.disabled=false;void refresh();}
   }
   async function refresh(){
     ...
     // Remove all old action buttons before any asynchronous authorization/status request.
     ...
     const formCodes=[...new Set(snapshots.map(x=>x.code))];
     const adminEntries=await Promise.all(formCodes.map(async code=>[code,await isFormAdmin(code)]));
     const admins=new Map(adminEntries);
     ...
     snapshots.forEach(({item,code,id},i)=>{
       ...
       // Only the explicit server-verified administrator for THIS form sees Admin Delete.
       if(admins.get(code)===true&&!state.claimPaid&&!['completed','disbursed','paid','claim paid'].includes(String(state.workflowStatus||'').toLowerCase())&&!pending.has('delete:'+code+':'+id)){
