...
let renderCalls=0,financeCalls=0,reads=0;
const rows=[
...
(async()=>{
 ...
 assert.equal(renderCalls,1);assert.equal(financeCalls,1);
 ...
 console.log('PASS Dashboard drawer: canonical badges, unknown fail-closed, original controls kept, read-only');
